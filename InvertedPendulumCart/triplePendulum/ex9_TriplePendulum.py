...
from pytrajectory.system import TransitionProblem
...
def n_bar_pendulum(N=1, param_values=dict()):
    ...
    if 'F' in param_values:
        F = param_values['F']
    elif F in param_values:
        F = param_values[F]
    
    if 'g' in param_values:
        g = param_values['g']
    elif g in param_values:
        g = param_values[g]
    else:
        g = 9.81
    
    for i, mi in enumerate(m):
        if mi.name in param_values:
            m[i] = param_values[mi.name]
        elif mi in param_values:
            m[i] = param_values[mi]
    
    for i, li in enumerate(l):
        if li.name in param_values:
            l[i] = param_values[li.name]
        elif li in param_values:
            l[i] = param_values[li]
